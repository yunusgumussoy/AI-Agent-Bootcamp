...
# Save as ollama_rag_lab_hybrid_v5.py and run: python ollama_rag_lab_hybrid_v5.py
...
# Gradio UI wiring
# -------------------------
with gr.Blocks(title="Local RAG Lab (v5 hybrid)") as demo:
    gr.Markdown("# 🔀 Local RAG Lab — v5 (Hybrid + Adaptive Retriever)")
    ...
    def on_upload_rebuild(files, chunk_size, chunk_overlap, top_k, embed_model, alpha):
        ...
        msg, indexed, status = ui_upload_and_rebuild(copied, int(chunk_size), int(chunk_overlap), int(top_k), embed_model, alpha)
        # update indexed choices
        choices = list(os.listdir(UPLOADED_DIR))
        return msg, choices, status

    upload_rebuild.click(fn=on_upload_rebuild, inputs=[files_in, chunk_size_in, chunk_overlap_in, top_k_in, gr.Textbox(value=EMBED_MODEL), alpha_in],
                         outputs=[chat, indexed, status_box])

    # Rebuild existing
    def on_rebuild_existing(chunk_size, chunk_overlap, top_k, embed_model, alpha):
        msg, indexed, status = ui_rebuild_existing(int(chunk_size), int(chunk_overlap), int(top_k), embed_model, alpha)
        choices = list(os.listdir(UPLOADED_DIR))
        return msg, choices, status

    rebuild_existing.click(fn=on_rebuild_existing, inputs=[chunk_size_in, chunk_overlap_in, top_k_in, gr.Textbox(value=EMBED_MODEL), alpha_in],
                           outputs=[chat, indexed, status_box])

    # delete
    def on_delete(selected, chunk_size, chunk_overlap, top_k, embed_model, alpha):
        msg, indexed, status = ui_delete_files(selected, int(chunk_size), int(chunk_overlap), int(top_k), embed_model, alpha)
        choices = list(os.listdir(UPLOADED_DIR))
        return msg, choices, status

    delete_btn.click(fn=on_delete, inputs=[indexed, chunk_size_in, chunk_overlap_in, top_k_in, gr.Textbox(value=EMBED_MODEL), alpha_in],
                     outputs=[chat, indexed, status_box])
